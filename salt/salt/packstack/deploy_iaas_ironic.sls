...
ironic_nodes = util.split_to_array(ironic_nodes, ',', [cc_node])
...
for node in ironic_nodes:
  ipic_list.append(hostname2ip[cc_node])
...
for hostname, ip in hostname2ip.items():
  ...
  if match:
    ipnc_list.append(ip)
ipnc_list = list( set(ipnc_list).difference(set(ipvm_list)) )
ipnc_list = ipic_list
...
iaas.installed:
  ...
  file.managed:
    ...
    - defaults:
      ...
      CINDER_BACKEND:        CONFIG_CINDER_BACKEND=lvm,${util.get_pillar('cinder_info:backend', "gluster")}
      VOLUMES_SIZE:          CONFIG_CINDER_VOLUMES_SIZE=${util.get_pillar('cinder_info:lvm_volumes_size', "10G")}
      ...
      IRONIC_INSTALL:        CONFIG_IRONIC_INSTALL=${util.get_pillar('ironic_info:install', "y")}
      VMWARE_BACKEND:        CONFIG_VMWARE_BACKEND=n
      VCENTER_HOST:          CONFIG_VCENTER_HOST=
      VCENTER_USER:          CONFIG_VCENTER_USER=
      VCENTER_PASSWORD:      CONFIG_VCENTER_PASSWORD=
      VCENTER_CLUSTER:       CONFIG_VCENTER_CLUSTER_NAME=
  grains.present:
    - name: exclude_ips
    - value: ${exclude_ips}
    - require:
